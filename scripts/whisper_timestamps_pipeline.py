#!/usr/bin/env python3
"""
Whisper-Timestamped Pipeline — openai/whisper-large-v3 (vanilla PyTorch backend)
Model: openai/whisper-large-v3
Output: /mnt/e/Corpus/ma/output/whisper_timestamps/{stem}.TextGrid
"""
...
OUT_DIR     = Path("/mnt/e/Corpus/ma/output/whisper_timestamps")
...
def main():
    ...
    print(f"[whisper_timestamps] {len(audio_files)} files | model={MODEL_SIZE} | device={DEVICE}")

    model = whisper.load_model(MODEL_SIZE, device=DEVICE)

    for af in tqdm(audio_files, desc="whisper_timestamps"):
        stem  = af.stem
