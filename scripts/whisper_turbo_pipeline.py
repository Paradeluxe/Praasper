#!/usr/bin/env python3
"""
Vanilla Whisper Pipeline — openai/whisper-large-v3 (PyTorch backend)
Model: openai/whisper-large-v3
Output: /mnt/e/Corpus/ma/output/whisper_turbo/{stem}.TextGrid
"""
...
OUT_DIR     = Path("/mnt/e/Corpus/ma/output/whisper_turbo")
...
def main():
    ...
    print(f"[whisper_turbo] {len(audio_files)} files | model={MODEL_SIZE} | device={DEVICE}")

    model = whisper.load_model(MODEL_SIZE, device=DEVICE)

    for af in tqdm(audio_files, desc="whisper_turbo"):
        stem  = af.stem
