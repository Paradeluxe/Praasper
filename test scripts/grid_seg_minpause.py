#!/usr/bin/env python3
"""Pilot grid search for seg_dur × min_pause on subjects 01-02 (8 files)."""
...
RESULTS_CSV = os.path.join(BASE, 'results', 'grid_seg_minpause_pilot.csv')

# ── Grid values ──
SEG_DUR_VALUES = [5, 10, 15, 20, 30]
MIN_PAUSE_VALUES = [0.1, 0.2, 0.3, 0.5]
...
for wav_fname in TARGET_FILES:
    ...
    try:
        # Run auto_vad ONCE to get best params for this file
...
# ── Step 2: Grid search seg_dur × min_pause using cached VAD params ──
print("\n" + "="*60)
print("STEP 2: Grid search seg_dur × min_pause (20 combos)")
print("="*60)

all_results = []

for seg_dur in SEG_DUR_VALUES:
    for min_pause in MIN_PAUSE_VALUES:
        clear_output()
        per_file_metrics = []
        ...
        for wav_fname in TARGET_FILES:
            ...
            wav_path = os.path.join(INPUT_DIR, wav_fname)
            try:
                # Run annotation with FIXED VAD params + structural combo
                model.annote(
                    input_path=wav_path,
                    seg_dur=seg_dur,
                    min_pause=min_pause,
                    params=file_best_params[wav_fname],  # Use cached best params
                    verbose=False
                )
                
                # Evaluate this file immediately
                ...
                if os.path.exists(out_path) and os.path.exists(ans_path):
                    ...
                    if res:
                        ...
                        print(f"  {wav_fname}: hit={res['hit']:.4f} eff={res['eff']:.4f} sWER={res['swer']:.4f}")
